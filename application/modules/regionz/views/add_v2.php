		...
		<div class="row">
			...
			<div class="d-flex">
				...
				<div class="btn-group" role="group">
					<a href="<?= site_url($this->uri->segment(1).'/method') ?>" class="btn btn-white btn-icon">
					<span class="btn-inner--icon"><i class="far fa-align-left"></i></span>
					<span class="btn-inner--text d-none d-md-inline-block">List</span>
					</a>
					<a href="<?= site_url($this->uri->segment(1).'/methodAdd') ?>" class="btn btn-white btn-icon active">
		...
		<!-- Shopping table table -->
			<div>
				<div class="row row-grid align-items-center">
					<div class="col-lg-6">
						<form id="regionForm">
							<h3>Tambahkan area UMKM</h3>
							<div class="form-group">
								<input type="text" class="form-control" id="name" name="name" placeholder="Masukkan area UKM (Pandaan, Purwosari, Sumberejo, dll)">
							</div>
							<div class="form-group">
								<input type="hidden" class="form-control" id="city" name="city" value="PASURUAN" placeholder="Masukkan kota/kabupaten">
							</div>
	...
	<script>
		$(document).ready(function(){
			$("#regionForm").submit(function(event) {
				...
				$.ajax({
					url: "<?= site_url($this->uri->segment(1)) ?>/proccessAdd",
					type: "POST",
					data: $(this).serialize(),
					dataType: "json",
					cache: false,
