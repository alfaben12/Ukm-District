...
<body>
		...
		<div class="row">
			...
			<div class="d-flex">
				...
				<div class="btn-group" role="group">
					<a href="<?= site_url('productz') ?>" class="btn btn-white btn-icon">
					<span class="btn-inner--icon"><i class="far fa-align-left"></i></span>
					<span class="btn-inner--text d-none d-md-inline-block">List</span>
					</a>
					<a href="<?= site_url('productz/add') ?>" class="btn btn-white btn-icon active">
					<span class="btn-inner--icon"><i class="far fa-sync"></i></span>
				...
				<div class="row row-grid align-items-center">
                    ...
                    foreach ($productUkm as $key => $val) {
						...
						<form id="productForm">
							<h3>Edit produk</h3>
							<div class="form-group">
                                <img src="<?= base_url() ?>files/product/<?= $val['image'] ?>" width="200" height="120"> Tidak dapat diubah
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<select name="ukm_category_product_id" id="ukm_category_product_id" data-toggle="select" title="Kategori" class="form-control" style="width:100%">
									...
									foreach ($category as $i => $cat) {
										?>
											<option value="<?= $cat['id'] ?>" <?= $val['ukm_category_product_id'] == $cat['id'] ? 'selected=""' : '' ?>><?= $cat['name'] ?></option>
										<?php
									}
									?>
								</select>
							...
							<div class="form-group">
								<select name="ukm_region_id" id="ukm_region_id" data-toggle="select" title="Region" class="form-control" style="width:100%">
									...
									<?php
									foreach ($region as $j => $reg) {
										?>
											<option value="<?= $reg['id'] ?>" <?= $val['region_id'] == $reg['id'] ? 'selected=""' : '' ?>><?= $reg['name'] ?></option>
										<?php
									}
									?>
								</select>
							...
							<div class="form-group">
								<input type="number" class="form-control" id="rating" name="rating" value="<?= $val['rating'] ?>" placeholder="Masukkan rating (1-5)" max="5">
							</div>
							<div class="form-group">
							...
							<div class="custom-control custom-checkbox">
								<input type="checkbox" class="custom-control-input" id="is_diskon" name="is_diskon" placeholder="Diskon?" data-toggle="indeterminate">
								<label class="custom-control-label" for="is_diskon">
								Apakah diskon?
								</label>
							</div>
							</div> -->
	...
	<script>
		$(document).ready(function(){
			...
			// 	if ($(this).is(':checked')) {
			// 		$('#final_price').attr('disabled', false);
			// 		var is_diskon_pars = 1;
			// 	}else{
			// 		$('#final_price').attr('disabled', true);
			// 		$('#final_price').val('');
			// 		var is_diskon_pars = 0;
			// 	}
			// });
			...
			$("#productForm").submit(function(event) {
				...
				var ukm_region_id = $('#ukm_region_id').val();
				var price = $('#price').val();
				var stock = $('#stock').val();
